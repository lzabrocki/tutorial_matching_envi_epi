...
source(here::here("docs", "2.functions",
...
We find that the average effect on the treated is equal to +`r results_ps_inf_caliper$estimate` years of life lost. The 95% confidence interval is consistent with effects ranging from +`r results_ps_inf_caliper$conf.low` up to +`r results_ps_inf_caliper$conf.high`. If we want to increase the precision of our estimate and remove any remaining imbalance in covariates, we can also run a multivariate regression. We adjust below for the same variables used in the estimation of propensity scores and the day of the week:
...
We find that the average effect on the treated is equal to +`r results_ps_inf_caliper_w_cov$estimate` years of life lost. The 95% confidence interval is consistent with effects ranging from +`r results_ps_inf_caliper_w_cov$conf.low` up to +`r results_ps_inf_caliper_w_cov$conf.high`. The width of confidence interval is now equal to `r results_ps_inf_caliper_w_cov$conf.high - results_ps_inf_caliper_w_cov$conf.low`, which is more than half smaller than the previous interval of `r results_ps_inf_caliper$conf.high - results_ps_inf_caliper$conf.low`.
...
The estimated is equal to +`r results_ps_0.5_caliper$estimate` years of life lost. The 95% confidence interval is consistent with effects ranging from +`r results_ps_0.5_caliper$conf.low` up to +`r results_ps_0.5_caliper$conf.high`. We finally run the regression model where we adjust for covariates:
...
We find that the average effect on the treated is equal to +`r results_ps_0.5_caliper_w_cov$estimate` years of life lost. The 95% confidence interval is consistent with effects ranging from +`r results_ps_0.5_caliper_w_cov$conf.low` up to +`r results_ps_0.5_caliper_w_cov$conf.high`. The width of confidence interval is now equal to `r results_ps_0.5_caliper_w_cov$conf.high - results_ps_0.5_caliper_w_cov$conf.low`, which is more than half smaller than the previous interval of `r results_ps_0.5_caliper$conf.high - results_ps_0.5_caliper$conf.low`.
