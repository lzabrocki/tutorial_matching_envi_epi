...
We find that the average effect on the treated is equal to +`r results_cpm_w_cov$estimate` years of life lost. The 95% confidence interval is consistent with effects ranging from +`r results_cpm_w_cov$conf.low` YLL up to +`r results_cpm_w_cov$conf.high` YLL. The width of confidence interval is now equal to `r results_cpm_w_cov$conf.high - results_cpm_w_cov$conf.low` YLL, which is just a bit smaller than the previous interval of `r results_cpm_wo_cov$conf.high - results_cpm_wo_cov$conf.low` YLL.
